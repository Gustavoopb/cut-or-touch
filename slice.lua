...
local earth = display.newRect( 0, 0, earthSize, earthSize)
...
borderLeft:setFillColor(0, 0, 255/255, 250/250)
...
local function moveRect()
	...
	transition.to(earth, {time = 2000, alpha = 1})
...
local function createSplit(maskXloc, maskYloc, angle1, angle2)
		...
		local earth1 = display.newRect( 0, 0, earthSize, earthSize)
		earth1.x = earth.x
		earth1.y = earth.y
		earth1:setMask( mask )
		physics.addBody( earth1, {isSensor = true} )

		local earth2 = display.newRect( 0, 0, earthSize, earthSize)
		earth2.x = earth.x
		earth2.y = earth.y
		earth2:setMask( mask )
...
local function onCollision(event)
	...
	if event.object2.name == "edge" then
		...
		transition.to(earth, {time = 100, alpha = 0} )
	end
...
transition.moveTo(earth, {x=400,y=400,time=3000})